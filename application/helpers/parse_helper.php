...
function get_images_src_array($source, $size)
    {
    $data = array();

    $images = parse_array($source, "<img", ">");

    for($xx=0; $xx<count($images); $xx++) {

        $data[] = get_attribute($images[$xx], $attribute = "src");
    }
    if(count($data)==0)$data[0] = "http://placehold.it/".$size."&text=no+image";
    return $data;
    }
